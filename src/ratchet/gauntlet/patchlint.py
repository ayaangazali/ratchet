...
The pawl already *reverts* protected paths before grading, so most of these cannot
change the outcome. patchlint exists for the other half of the job: telling the
human, loudly, that the agent tried. CRITICAL findings disqualify the attempt.
...
_RE_RULES: list[tuple[str, re.Pattern[str], Severity, str]] = [
    ...
    (
        "skip_marker",
        re.compile(r"@?pytest\.mark\.(skip|skipif|xfail)|unittest\.skip|self\.skipTest|pytest\.skip\s*\("),
        Severity.CRITICAL,
...
_ASSERT_RE = re.compile(r"^\s*(assert\b|self\.assert\w+\()")
...
def lint(
    ...
    *,
    ...
    for f in files:
        removed_tests = [(ln, t) for ln, t in f.removed if _DEF_TEST.match(t)]
        added_tests = {_DEF_TEST.match(t).group(2) for _, t in f.added if _DEF_TEST.match(t)}
        for ln, t in removed_tests:
            name = _DEF_TEST.match(t).group(2)
