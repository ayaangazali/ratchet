...
class ReceiptBook:
    ...
    def record(self, verdict: Verdict) -> Receipt:
        chain = self.all()
        ...
        r = Receipt(
            ...
            verdict_digest=hashlib.sha256(
                json.dumps(verdict.to_dict(), sort_keys=True, separators=(",", ":"), default=str).encode()
            ).hexdigest(),
    ...
    def verify(self) -> tuple[bool, list[str]]:
        ...
        for i, r in enumerate(self.all()):
            ...
            if r.prev != prev:
                problems.append(f"receipt {i} ({r.attempt_id}): chain broken -- prev {r.prev[:12]} != {prev[:12]}")
            if not hmac.compare_digest(r.sig, self._sign(r)):
                problems.append(f"receipt {i} ({r.attempt_id}): signature does not verify")
            prev = r.digest()
