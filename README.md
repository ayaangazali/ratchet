...
costing and what you can do about it. It renders entirely off the JSONL bus, so it
can be started, killed and restarted mid-run, and a finished run replays into it:
...
If the console dies mid-demo, the approval still works with one shell command:
`echo '{"allow": true}' > demo-repo/.ratchet/approvals/<id>.json` — the gate is a
file, deliberately, because a demo that can be rescued with `echo` is a demo that
survives.
